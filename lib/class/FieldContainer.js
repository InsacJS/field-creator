...
/** @ignore */ const Field = require('./Field.js')
...
/**
* Normaliza las propiedades de un modelo. Actualiza los mensajes de error,
* según el attributo y modelo al que pertenece.
* @param {SequelizeModel} MODEL Modelo Sequelize.
*/
function _normalize (MODEL) {
  function _normalizeField (field, fieldName) {
    Field.normalize(field)
    Object.keys(field.validate).forEach(key => {
      const ARGS = field.validate[key].args
      const MSG = field.validate[key].msg
      field.validate[key].msg = MSG || _createErrorMsg(key, ARGS, fieldName, MODEL.name)
    })
  }
  Object.keys(MODEL.attributes).forEach(key => {
    _normalizeField(MODEL.attributes[key], key)
  })
}

/**
* Crea un mensaje de error apropiado para un tipo de validador específico.
* @param {String} valKey Nombre del validador.
* @param {Object[]} ARGS Argumentos del validador.
* @param {String} fieldName Nombre del atributo.
* @param {String} modelName Nombre del modelo.
*/
function _createErrorMsg (valKey, ARGS, fieldName, modelName) {
  let msg
  if (valKey === 'isLen') { msg = `El campo '{{fieldName}}' del modelo '{{modelName}}' debe tener entre {{args0}} y {{args1}} caracteres.` }
  if (valKey === 'isInt') { msg = `El campo '{{fieldName}}' del modelo '{{modelName}}' debe ser un número de tipo Integer.` }
  if (valKey === 'isFloat') { msg = `El campo '{{fieldName}}' del modelo '{{modelName}}' debe ser un número de tipo Float.` }
  if (valKey === 'min') { msg = `El campo '{{fieldName}}' del modelo '{{modelName}}' debe ser menor o igual a {{args0}}.` }
  if (valKey === 'max') { msg = `El campo '{{fieldName}}' del modelo '{{modelName}}' debe ser menor o igual a {{args0}}.` }
  if (valKey === 'isIn') { msg = `El campo '{{fieldName}}' del modelo '{{modelName}}' debe encontrarse entre los valores: {{args0}}.` }
  const data = { fieldName, modelName }
  for (let i = 0; i < ARGS.length; i++) { data[`args${i}`] = ARGS[i].toString() }
