...
class FieldContainer {
  ...
  */
  static THIS (arg1, arg2) {
    ...
    if (modelName) { properties._thisModel = modelName }
...
function _updateTHIS (MODEL, obj, modelsFunction) {
  ...
  Object.keys(obj).forEach(prop => {
    const OBJ = obj[prop]
    if (_isTHIS(OBJ)) {
      const modelName = OBJ._thisModel || (MODEL ? MODEL.name : 'UNDEFINED')
      if (modelName === 'UNDEFINED') {
        throw new Error(`No se definió un modelo para el campo '${prop}'.`)
      }
      if (!modelsFunction[modelName]) {
        throw new Error(`EL modelo '${modelName}' no se encuentra definido.`)
      }
...
function _createModelFunction (MODEL) {
  return (fieldName, properties = {}) => {
    ...
    return Object.assign(_.cloneDeep(MODEL.attributes[fieldName]), properties)
  }
}
