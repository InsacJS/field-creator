...
class Field {
  ...
  */
  static add (name, field, options = {}) {
    options.force = typeof options.force === 'boolean' ? options.force : false
    const NAME = name.toUpperCase()
    if (typeof Field[NAME] !== 'undefined') {
      if (!options.force) {
        throw new Error(`El tipo de dato '${NAME}' ya ha sido definido`)
      }
    }
    Field[NAME] = (properties = {}) => { return Field.clone(field, properties) }
  }

  /**
  * Adiciona o modifica un tipo de dato.
  * @param {!String} name                    - Nombre con el que se identificará al nuevo tipo de campo.
  * @param {!Field}  field                   - Instancia de la clase Field.
  * @param {Boolean} [options.force = false] - Indica si se sobreescribirá el tipo de dato anterior, si es que hubiese.
  */
  static use (name, field) {
    const NAME = name.toUpperCase()
    Field[NAME] = (properties = {}) => { return Field.clone(field, properties) }
  }
...
function _updateTHIS (model, obj, fullPath = '', baseModel) {
  ...
  Object.keys(obj).forEach(prop => {
    ...
    if (_isTHIS(OBJ)) {
      if (!model || !(model.attributes || model.rawAttributes)[prop]) {
        const msg = `El campo '${fullPath2}: THIS()' no es parte del modelo ${baseModel ? `'${baseModel.name}'` : `'${model ? model.name : 'null'}'`}\n`
        throw new Error(msg)
      }
      delete OBJ._this
      RESULT[prop] = Object.assign(_.cloneDeep((model.attributes || model.rawAttributes)[prop]), OBJ)
      return
    }
    if (_isField(OBJ)) {
...
function _validateGroup (obj, fullPath = '') {
  ...
  Object.keys(obj).forEach(prop => {
    ...
    const fullPath2 = `${fullPath ? `${fullPath}.` : ''}${prop}`
    if (_isTHIS(OBJ)) {
      const msg = `El campo '${fullPath2}: THIS()' debe ser parte de un modelo\n`
      throw new Error(msg)
    }
    if (typeof OBJ === 'object') {
...
function _normalizeValidate (field) {
  if (field.validate) {
    Object.keys(field.validate).forEach(key => {
      const validateItem = field.validate[key]
...
module.exports = require('./CustomTypes')(Field)
